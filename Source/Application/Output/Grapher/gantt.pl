...
gantt:emit_js_state :-
    ...
    write('let hoverMode=false, hoverPkg=null, critMode=false;'), nl.
...
gantt:emit_js_functions :-
    ...
    write('  const edges=[],stepOf=c=>c.closest("td").cellIndex;'), nl,
    ...
    write('  drawOverlays();'), nl,
    write('}'), nl,
    write('function markCritical(edges,stepOf){'), nl,
    write('  document.querySelectorAll("#gantt .cell.crit").forEach(c=>c.classList.remove("crit"));'), nl,
    ...
    write('  const order=[...nodes].sort((a,b)=>stepOf(a)-stepOf(b));'), nl,
    write('  order.forEach(n=>len.set(n,1));'), nl,
    write('  edges.slice().sort((a,b)=>stepOf(a.from)-stepOf(b.from)).forEach(e=>{'), nl,
    write('    const l=len.get(e.from)+1;if(l>len.get(e.to)){len.set(e.to,l);pred.set(e.to,e);}'), nl,
    write('  });'), nl,
    write('  let end=null;'), nl,
    write('  order.forEach(n=>{if(!end||len.get(n)>len.get(end)||(len.get(n)===len.get(end)&&stepOf(n)>stepOf(end)))end=n;});'), nl,
    write('  const n=end?len.get(end):0;btn.textContent=`Critical path (${n})`;'), nl,
    write('  if(n<2)return;'), nl,
    write('  for(let c=end;c;){c.classList.add("crit");const e=pred.get(c);if(!e)break;e.el.classList.add("crit");c=e.from;}'), nl,
    write('}'), nl,
    ...
    write('  document.querySelectorAll("#gantt tr.hover-lit").forEach(r=>r.classList.remove("hover-lit"));'), nl,
    ...
    write('  const r=document.querySelector(`#gantt tr[data-pkg="${p}"]`);if(r)r.classList.add("hover-lit");'), nl,
    write('}'), nl,
    write('(function(){'), nl,
    write('  const tb=document.querySelector("#gantt tbody");if(!tb)return;'), nl,
    write('  tb.addEventListener("mouseover",e=>{'), nl,
    write('    if(!hoverMode)return;'), nl,
    write('    const r=e.target.closest("tr[data-pkg],tr.detail-row"),p=r?(r.dataset.pkg||r.dataset.parent):null;'), nl,
    write('    if(p!==hoverPkg)litEdges(p);'), nl,
    write('  });'), nl,
    write('  tb.addEventListener("mouseleave",()=>{if(hoverMode)litEdges(null);});'), nl,
    write('})();'), nl,
