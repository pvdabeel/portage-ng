...
| **--search-bugs** | Search for bugs matching the given term. With a synced `bugzilla` repository the local bug store (`Knowledge/bugs.qlf`) answers first and the live Gentoo Bugzilla REST quicksearch is the fallback; `config:bugzilla_search(rest)` queries Bugzilla directly. |
...
| **--optimize** *parallelism*\|*soft-requirements* | Plan ordering strategy. Hard requirements are always satisfied; the strategies differ in how soft requirements (preferences such as RDEPEND-before-consumer) that contradict each other are handled. `parallelism` (default): a preference whose provider depends back on its consumer is void, so the members of a runtime cycle merge side by side in one step; preferences between packages not on a common cycle are always honored. `soft-requirements`: every preference is handed to the planner, which honors as many as it can and drops only the ones that would close a cycle; cycle members merge one per step, resembling `emerge`'s linear listing. See the handbook's Planning chapter. |
...
| `Knowledge/bugs.qlf` | Local Bugzilla bug store of a registered `bugzilla`-type repository (generated by `--sync`; the network step is capped by `config:repository_sync_limit/2`). |
| `Knowledge/<repository>.sync` | Time stamps of recent network syncs for repositories with a `config:repository_sync_limit/2` cap. |
