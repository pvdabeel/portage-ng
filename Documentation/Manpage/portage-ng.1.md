...
| **--optimize** *parallelism*\|*soft-requirements* | Plan ordering strategy. Hard requirements are always satisfied; the strategies differ in how soft requirements (runtime-order wishes such as RDEPEND-before-consumer) that contradict each other are handled. `parallelism` (default): a wish whose provider depends back on its consumer is void, so the members of a runtime cycle merge side by side in one step; wishes between packages not on a common cycle are always granted. `soft-requirements`: every wish is handed to the planner, which grants as many as it can and drops only the ones that would close a cycle; cycle members merge one per step, resembling `emerge`'s linear listing. See the handbook's Planning chapter. |
